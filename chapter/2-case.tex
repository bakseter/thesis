...
\begin{enumerate}
    \item an equational theory of semilattices with one inflationary endomorphism
          has a uniform word problem that can be solved in polynomial time.
    \item loop-checking in a type system with universe-level constraints
          can be done in polynomial time.
\end{enumerate}

\textcolor{red}{first problem is maybe summarized to generally, while second is more specific for the case of type systems? ask Marc.}

We focused on formalizing the proof of \Cref{thm:3.2} from \cite{mbezem}.
...
called their join., denoted by $\lor$.
...
We denote the inflationary endomorphism as $\underline{}^+$, for every lattice element $x$,
$x^+$ is also called the successor of $x$.
The fact that $x$ is an endomorphism means that we get the folloring properties
...
\label{thm:3.2}
For any finite semilattice representation
...
\begin{customlem}{Lemma 3.3}[{\cite[p.~3-4]{mbezem}}]
\label{lem:3.3}
Given a finite semilattive presentation
