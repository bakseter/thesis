...
\textcolor{red}{would have been nice to explain this earlier or tie it in better with section about depdendent types in chap 1?}
...
Since (in our case) the formal definitions of \Cref{lem:3.3}, which will be expanded on shortly,
and \Cref{thm:3.2} share some structure, we define (with \lstinline{Set}) a proposition \lstinline{pre_thm}:
...
\Cref{lem:3.3} from \cite{mbezem} is used in the proof of \Cref{thm:3.2} to solve \textcolor{red}{fill inn explanation here...}
...
The proof of \Cref{thm:3.2} is based on a primary induction on \lstinline{n} and a secondary induction on \lstinline{m}.
...
The inductive case of \lstinline{m} by far the largest part of the proof (see lines 17-199 \Cref{lst:thm_32_full_proof}).
...
We will use sometimes use mathematical notation to explain the proof instead of Coq code,
