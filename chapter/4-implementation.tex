...
When using \lstinline{Prop}, we define it as a predicate, using standard first-order logic syntax.

When using \lstinline{Set}, we define it as a type,
using the \lstinline{sig} type constructor in place of \lstinline{exists}.
We also use the \lstinline{prod} type constructor to represent the conjunction of two predicates.
...
The former is used to define a predicate, while the latter is (usually) used to define a type or a non-recursive function.
...
The syntax of the first goal is a bit strange, but it is simply a predicate
